...
const EditHSNCode = () => {
  ...
  // Fetch HSN code by ID on mount
  useEffect(() => {
 const fetchHSN = async () => {
setFetching(true);
try {
      const response = await axios.get(`${Config.Search_HSN_url}/${id}`, {
        headers: {
...
      // Update this block as needed:
      const hsn =
        response.data.apiData
          ? (Array.isArray(response.data.apiData)
                ? response.data.apiData[0]
                : response.data.apiData)
          : response.data.apiData;
...
      } else {
        toast({
          description: "Failed to fetch HSN Code details.",
          status: "error", duration: 2000, isClosable: true
        });
      }
    } catch (err) {
      toast({
        description: err?.response?.data?.message || "Failed to load HSN code.",
        status: "error", duration: 2000, isClosable: true
      });
    } finally {
      setFetching(false);
...
  if (id) fetchHSN();
  ...
  // Update HSN Code
  const handleUpdateHSNCode = async () => {
    setLoading(true);
    try {
      if (!hsnFormData.HSN_CD) {
        toast({
          description: "Please fill in HSN Code field",
          duration: 2000, position: "bottom-center", status: "error", isClosable: true
        });
        setLoading(false);
        return;
      }
      ...
      const response = await axios.put(`${Config.Edit_HSN_url}/${id}`, apiData, {
        headers: {
      ...
      if (response?.status === 200 || response?.status === 201) {
        toast({
          description: response?.data?.message || "HSN Code updated successfully",
          duration: 2000, position: "bottom-center", status: "success", isClosable: true
        });
        setTimeout(() => {
          navigate('/hsn');
        }, 1500);
      ...
      toast({
        description: error?.response?.data?.message || "Failed to update HSN Code",
        duration: 2000, position: "bottom-center", status: "error", isClosable: true
      });
    } finally {
      setLoading(false);
            ...
            <Box width='100%' maxWidth='1200px' mx="auto">
              ...
              {fetching ? (
                <Flex justify="center" align="center" h="200px">
                  <Spinner size="lg" color="blue.600" mr={4} />
                  <Text>Loading HSN code...</Text>
                </Flex>
              ) : (
