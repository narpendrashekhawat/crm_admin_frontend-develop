...
import {
    ...
    BreadcrumbLink, Image, useDisclosure, TableContainer,Center,} from '@chakra-ui/react';
...
const handleApplyFilter = () => {
       ...
       const formattedEndDate = endDate ? dayjs(endDate).format("YYYY-MM-DD") : '';
        ...
        try {
            ...
            if (response?.status === 200) {
                setMappingList(response?.data?.data?.headers);
                setTotalItems(response?.data?.data?.totalHeaders);
                setTotalPages(response?.data?.data?.totalPages)
                console.log(response?.data?.data?.headers, 'headersList')
            }
    ...
    function handleNav(id, userName, userType) {
        ...
        nav(`/mapping/Mappingmap/${id}`, { state: { userName, userType } });
    };
    ...
    return (
        <>
            <FilterbyDate isDatepickerOpen={isDatepickerOpen} setIsDatepickerOpen={setIsDatepickerOpen} startDate={startDate} setStartDate={setStartDate} endDate={endDate} setEndDate={setEndDate} handleApplyFilter={handleApplyFilter}  />
