...
// Fix the icon imports to use the correct path
  ...
  return (
    <Box flex="1" width="300px">
      <Text fontSize="12px" color="#333333" mt="4" display="flex">
        Number of employees{" "}
        <Text color="#8B8D97" fontSize="12px" ml="1">
          (Optional)
        </Text>
      </Text>
      <Flex gap={2} mt={2}>
...
const AddRetailer = () => {
  ...
  console.log(employees, "employes");
  ...
  const addManufacturer = useCallback(async () => {
    ...
    try {
      ...
      console.log(empMin, empMax, "qwertyuytre");
      ...
      // API endpoint comment: This endpoint registers a new manufacturer in the system
      // It requires company name and email, while phone and employee count are optional
      // On success, credentials are sent to the provided email address

      // API call commented out as requested
      const response = await axios.post(
        `${Config?.AddRetailer_url}`,
        manufacturerData,
        {
          headers: {
      ...
      // Just simulate success for now (since API is commented out)
  ...
  const handledlNoChange = (e) => {
    setDlNo(e.target.value);
    if (errors.dlNo) {
      setErrors({ ...errors, dlNo: "" });
    }
  };
        ...
        <Box width="80%">
          ...
          <Box p={4} bg="white" mt="1rem" padding="12px 20px">
            ...
            <VStack padding="32px" spacing={6}>
              ...
              <HStack gap={"5rem"} alignItems="flex-start">
                <FormControl isInvalid={Boolean(errors.dlNo)} flex="1">
                  <FormLabel fontSize="12px" color="#333333">
                    DL No
                  </FormLabel>
                  <Input
                    value={dlNo}
                    onChange={handledlNoChange}
                  ...
                  />
                  {errors.dlNo && (
                    <FormErrorMessage>{errors.dlNo}</FormErrorMessage>
                  )}
                </FormControl>
                  ...
                  <HStack align="flex-start" width="100%" spacing={3}>
                    ...
                    <VStack alignItems="flex-start" spacing={1}>
                      ...
                      <Text fontSize="12px" lineHeight="1.4">
                        Please ask the client to check
                        <br />
                        the provided email for Username,
                        <br />
