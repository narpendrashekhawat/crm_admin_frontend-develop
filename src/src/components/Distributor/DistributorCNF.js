    ...
    const fetchDistributorStats = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                setStatsData({
                    Distributor: { val: data?.distributorStats?.totalDistributors?.count || 0, per: data?.distributorStats?.totalDistributors?.percentageChange },
                    activeDistributors: { val: data?.distributorStats?.activeDistributors?.count || 0, per: data?.distributorStats?.activeDistributors?.percentageChange },
                    inactiveDistributor: { val: data?.distributorStats?.inactiveDistributors?.count || 0, per: data?.distributorStats?.inactiveDistributors?.percentageChange },
                    CNF: { val: data?.productStats?.newProductsAdded || 0, per: data?.distributorStats?.totalDistributors?.percentageChange },
                    productsAdded: data?.productStats?.totalProducts || 0,
                    inactive: data?.productStats?.manufacturersWithoutDistributors || 0
                });
    ...
    return (
        ...
        <Box p={4} bg="white" mt='1rem' padding='12px 20px' borderRadius='15px 15px 0px 0px'>
            <Breadcrumb color="#8B8D97" padding='10px 0px 2rem 0px' >
                ...
                <BreadcrumbItem>
                    <BreadcrumbLink href='/manufacturers' color='#8B8D97' fontSize='13px'>Distributor / CNF List</BreadcrumbLink>
                </BreadcrumbItem>
            </Breadcrumb>
            ...
            <HStack justifyContent='space-between' gap="20px">
                ...
                <HStack>
                    <Button as={"a"} href="/AddDistributor" justifyContent={'space-between'} bg='#3E60AA' color='white' padding='0px 12px' fontWeight='400' borderRadius='12px' w='213px' h='36px' fontSize='14px'>
                    ...
                    <HStack justifyContent='space-between' width='87%'>
                        ...
                        <VStack gap='1px' alignItems='flex-start'>
                            ...
                            <HStack>
                                ...
                                <Text color='#45464E' fontSize='12px' fontWeight='500' lineHeight='26px' textColor={"#CC5F5F"}>{statsData?.inactiveDistributor?.per}</Text>
                            </HStack>
                        </VStack>
                    </HStack>
                </Box>
