...
const ProductCatalog = () => {
    ...
    const fetchProductStats = async () => {
        try {
            ...
            if (response.data && response.data.status) {
                setProductStats({
                    totalProducts: response.data.All_Products || 0,
                    unlockedProducts: response.data.unlocked_Products || 0,
                    lockedProducts: response.data.locked_Products || 0,
                    lastUpdated: response.data.last_Updated || 0
                });
            } else {
    ...
    const fetchProducts = async () => {
        try {
            ...
            setTotalPages(Math.ceil((response.data.totalCount || 0) / itemsPerPage) || 1);
                ...
                <HStack flex={2} >
                    ...
                    <Flex flex={1} bg="#F0F4F9" borderRadius="12px" h="93px" py={5} px={4} justify="space-between" marginRight="20px">
                        ...
                        <Box>
                            ...
                            <Text fontSize="16px" fontWeight="medium" color="#45464E" pt={2}>{currentDate}</Text>
                        </Box>
                    </Flex>
                </HStack>
                    ...
                    <Flex justify="space-between" align="center" mt={4}>
                        <Flex align="center">
                            ...
                            <Text fontSize="14px" color="#666666" marginLeft="12px">
                                {Math.min(1 + (currentPage - 1) * itemsPerPage, totalProducts)}-
                                {Math.min(currentPage * itemsPerPage, totalProducts)} of {totalProducts} items
                            </Text>
                        </Flex>
                        ...
                        <Flex align="center">
                            ...
                            <Text fontSize="14px" color="#666666" marginLeft="8px">of {totalProducts} pages</Text>
                            <IconButton
                                icon={<FiChevronLeft />}
                                variant="ghost"
                                size="sm"
                            ...
                            <IconButton
                                ...
                                isDisabled={currentPage === totalProducts}
                            />
                        </Flex>
                    </Flex>
                )}
